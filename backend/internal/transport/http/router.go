...
func NewRouter(handler *Handler, hlsDir string) *mux.Router {
	...
	api.HandleFunc("/torrent/start/{id}", handler.StartTorrent).Methods("POST")
	api.HandleFunc("/torrent/stop/{id}", handler.StopTorrent).Methods("POST")
	...
	api.HandleFunc("/watch-hubs/ping", handler.PingWatchHub).Methods("POST")
